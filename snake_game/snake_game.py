...
#폰트
font_path = "C:/Users/USER/Documents/Introduction to visual media programming/snake_game/assets/NanumGothicCoding-Bold.ttf"
...
class Snake(object):
    ...
    def create(self):
        ...
        self.red = random.randint(100,200)
        self.green = random.randint(100,200)
        self.blue = random.randint(100,200)
    ...
    def draw(self, screen):
        red, green, blue = int(self.red / (self.length - 1)), int(self.green/(self.length - 1)), int(self.blue / (self.length - 1))
...
class Game(object):
    ...
    def run_logic(self):
        self.snake.move()
        self.check_eat(self.snake, self.feed[0])
        self.check_eat(self.snake, self.feed[1])
    ...
    def check_eat(self, snake, feed):
        if snake.positions[0] == feed.position:
            snake.eat()
            feed.create()
            if self.hscore < self.snake.length:
                self.hscore = self.snake.length
    ...
    def draw_info(self, length, speed, screen):
        ...
        info = "Length: " + str(length) + "    " + "Speed: " + str(round(speed, 2)) + "    "+"Highest Score: " + str(self.hscore)
        #font_path = resource_path("assets/NanumGothicCoding-Bold.ttf")
        font = pygame.font.Font(font_path, 26)
        text_obj = font.render(info, 0.5, GRAY)
    ...
    def display_frame(self, screen):
        ...
        self.feed[0].draw(screen)
        self.feed[1].draw(screen)
